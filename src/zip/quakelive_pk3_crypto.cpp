...
namespace {
// Quake Live Beta PK3 XOR table derived from Luigi Auriemma's quakelivedec.c (GPLv2+).
static constexpr char kQuakeLiveXor[] =
...
constexpr int kQuakeLiveXorLen = static_cast<int>(sizeof(kQuakeLiveXor) - 1);
}  // namespace

bool looks_like_quakelive_encrypted_zip_header(const QString& file_path) {
	QFile f(file_path);
	if (!f.open(QIODevice::ReadOnly)) {
		return false;
	}
	const QByteArray head = f.read(4);
	if (head.size() != 4 || kQuakeLiveXorLen < 4) {
		return false;
	}

	uchar d[4] = {};
	for (int i = 0; i < 4; ++i) {
		const uchar b = static_cast<uchar>(head[i]);
		const uchar k = static_cast<uchar>(kQuakeLiveXor[i]);
		d[i] = static_cast<uchar>(b ^ k);
	}
	if (d[0] != 'P' || d[1] != 'K') {
		return false;
	}
	if ((d[2] == 0x03 && d[3] == 0x04) ||
		(d[2] == 0x05 && d[3] == 0x06) ||
		(d[2] == 0x07 && d[3] == 0x08)) {
		return true;
	}
...
bool quakelive_pk3_xor_stream(QIODevice& in, QIODevice& out, QString* error) {
	if (error) {
		error->clear();
	}
	if (kQuakeLiveXorLen <= 0) {
		if (error) {
			*error = "Quake Live XOR table is missing.";
		}
		return false;
	}
	...
	qint64 pos = 0;
	for (;;) {
		const qint64 got = in.read(buf.data(), buf.size());
		if (got < 0) {
			if (error) {
		...
		for (qint64 i = 0; i < got; ++i) {
			const int ki = static_cast<int>((pos + i) % kQuakeLiveXorLen);
			buf[static_cast<int>(i)] = static_cast<char>(
				static_cast<uchar>(buf[static_cast<int>(i)]) ^
				static_cast<uchar>(kQuakeLiveXor[ki]));
		}
		if (out.write(buf.constData(), got) != got) {
			if (error) {
				*error = "Unable to write output stream.";
			}
			return false;
		}
		pos += got;
