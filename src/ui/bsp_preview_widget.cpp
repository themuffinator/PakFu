...
void BspPreviewWidget::focusOutEvent(QFocusEvent* event) {
  fly_timer_.stop();
  fly_move_mask_ = 0;
  if (drag_mode_ == DragMode::Look) {
    releaseMouse();
...
void BspPreviewWidget::on_fly_tick() {
  if (drag_mode_ != DragMode::Look) {
    fly_timer_.stop();
    fly_move_mask_ = 0;
    return;
...
void BspPreviewWidget::set_fly_key(int key, bool down) {
  int mask = 0;
  switch (key) {
    case Qt::Key_W:
    case Qt::Key_Up:
...
void BspPreviewWidget::ensure_grid_program() {
  if (grid_program_.isLinked()) {
    return;
  }
  ...
  if (!vs_ok || !fs_ok || !grid_program_.link()) {
    qWarning() << "BspPreviewWidget grid shader compile/link failed:" << grid_program_.log();
  }
}
...
void BspPreviewWidget::update_grid_lines_if_needed(const QVector3D& cam_pos, float aspect) {
